    ...
    <footer className="border-t border-[#DAD8D2] bg-[#102A20] text-white">
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div><h3 className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[#78D5A6]">{t('capabilities')}</h3><ul className="mt-5 space-y-3 text-sm text-white/65">{services.slice(0, 5).map(service => <li key={service.slug}><Link className="hover:text-white" href={localizedPath(lang, `/services/${service.slug}`)}>{service.title[lang]}</Link></li>)}</ul></div>
          <div><h3 className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[#78D5A6]">{t('platform')}</h3><ul className="mt-5 space-y-3 text-sm text-white/65"><li><Link className="hover:text-white" href={localizedPath(lang, '/work')}>{nav('showcase')}</Link></li><li><Link className="hover:text-white" href={localizedPath(lang, '/pricing')}>{nav('pricing')}</Link></li><li><Link className="hover:text-white" href={localizedPath(lang, '/guides')}>{nav('guides')}</Link></li><li><Link className="hover:text-white" href={localizedPath(lang, '/faq')}>{nav('faq')}</Link></li><li><Link className="font-bold text-[#78D5A6]" href={localizedPath(lang, '/quote')}>{t('quote')}</Link></li><li><Link className="hover:text-white" href="/dashboard">{t('portal')}</Link></li></ul></div>
        </div>
