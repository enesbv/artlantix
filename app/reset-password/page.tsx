...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (password.length < 8) return setMessage(t('lengthError'));
      ...
      <main className="mx-auto max-w-md px-4 py-16">
        <form onSubmit={handleSubmit} className="rounded-2xl border border-[#E6E4DF] bg-white p-8 shadow-xs">
          ...
          {!success && <div className="mt-6 space-y-4">
            <label className="block text-xs font-bold">{t('newPassword')}<input type="password" autoComplete="new-password" minLength={8} required value={password} onChange={(event) => setPassword(event.target.value)} className="mt-1 block w-full rounded-lg border border-[#E6E4DF] px-3 py-2" /></label>
            <label className="block text-xs font-bold">{t('confirm')}<input type="password" autoComplete="new-password" minLength={8} required value={confirmation} onChange={(event) => setConfirmation(event.target.value)} className="mt-1 block w-full rounded-lg border border-[#E6E4DF] px-3 py-2" /></label>
